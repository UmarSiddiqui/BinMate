/**
 * Seed City of South Perth council + 10 collection zones.
 * Run with: npx tsx prisma/seed-southperth.ts
 *
 * Zone code convention: COSP-{DAY_ABBREV}-{RECYCLING_WEEK}
 * All 5 weekdays × 2 recycling weeks (A and B) = 10 zones.
 *
 * Zone schema (South Perth 2-bin system at March 2026):
 *   generalDay / generalFrequency = 'weekly'  → green lid general waste (weekly)
 *   recyclingDay / recyclingWeek = A | B       → yellow lid recycling (fortnightly)
 *   greenWasteDay / greenWasteWeek = null      → no FOGO/green waste (Verge Valet™ replaces it)
 *   vergeDates = null                          → Verge Valet™ is on-demand, not calendar-based
 *
 * Zones confirmed from live API testing on 2026-03-16:
 *   COSP-TUE-A  1 Sandgate Street SOUTH PERTH WA 6151
 *               → "Waste Pickup Day": "Every Tuesday", "Next Recycling Pickup": "17 March 2026" → Week A
 */

import { PrismaClient } from '@prisma/client';

const prisma = new PrismaClient();

const DAYS = [
  { abbrev: 'MON', day: 'monday'    },
  { abbrev: 'TUE', day: 'tuesday'   },
  { abbrev: 'WED', day: 'wednesday' },
  { abbrev: 'THU', day: 'thursday'  },
  { abbrev: 'FRI', day: 'friday'    },
] as const;

const WEEKS = ['A', 'B'] as const;

async function main(): Promise<void> {
  console.log('Seeding City of South Perth...');

  const council = await prisma.council.upsert({
    where: { slug: 'south-perth' },
    update: { isActive: true },
    create: {
      name: 'City of South Perth',
      slug: 'south-perth',
      platformType: 't1cloud',
      apiEndpoint: 'https://cosp.spatial.t1cloud.com/spatial/IntraMaps/ApplicationEngine',
      isActive: true,
    },
  });

  console.log(`  ✓ Council: ${council.name} (${council.id})`);

  let count = 0;

  for (const { abbrev, day } of DAYS) {
    for (const recyclingWeek of WEEKS) {
      const zoneCode = `COSP-${abbrev}-${recyclingWeek}`;
      const dayLabel = day.charAt(0).toUpperCase() + day.slice(1);
      const zoneName = `City of South Perth — ${dayLabel} (recycling Week ${recyclingWeek})`;

      const data = {
        zoneName,
        generalDay:       day,
        generalFrequency: 'weekly',
        recyclingDay:     day,
        recyclingWeek,
        greenWasteDay:    null,   // no FOGO/green waste service
        greenWasteWeek:   null,
        updatedAt:        new Date(),
      };

      const existing = await prisma.collectionZone.findFirst({
        where: { councilId: council.id, zoneCode },
      });

      if (existing) {
        await prisma.collectionZone.update({ where: { id: existing.id }, data });
      } else {
        await prisma.collectionZone.create({
          data: { councilId: council.id, zoneCode, ...data },
        });
      }

      console.log(`  ✓ Zone: ${zoneCode} (${day}, recycling ${recyclingWeek})`);
      count++;
    }
  }

  console.log(`\nSeeded ${count} South Perth zones.`);
}

main()
  .catch((err) => { console.error('Seed failed:', err); process.exit(1); })
  .finally(() => prisma.$disconnect());
